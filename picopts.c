...
// Copyright (c) Andrew Herbert - 26/08/2022

// MIT Licence.

// NB LOGGING FORCED TRUE!
...
static void pts_emulation()
{
  ...
  while ( TRUE ) {
    ...
    if ( (request = gpio_get_all()) & RDRREQ_BIT ) {
      ...
      // teletype input can be interrupted by a computer restart
      if ( (request & TTYSEL_BIT) && ( data == 255 ) ) {
	while ( (data = getchar_timeout_us(1000)) == PICO_ERROR_TIMEOUT );
	if ( data == 255 )
	...
	put_pts_ch(data, request);
      ...
      putchar_raw(get_pts_ch(request));
    };
  };
}
